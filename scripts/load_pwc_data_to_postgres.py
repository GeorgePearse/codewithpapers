...
def load_papers(conn, batch_size=1000, checkpoint=None):
    ...
    for i in range(start_offset, len(df), batch_size):
        ...
        print(
            f"  Processed {min(i + batch_size, len(df))}/{len(df)} papers ({inserted} inserted, {skipped} skipped)"
        )
    ...
    cur.close()
    print(
        f"✓ Papers loaded: {inserted} inserted, {skipped} skipped (duplicates or missing arxiv_id)"
    )
    return inserted
...
def load_datasets(conn, batch_size=1000, checkpoint=None):
    ...
    for i in range(start_offset, len(df), batch_size):
        ...
        print(
            f"  Processed {min(i + batch_size, len(df))}/{len(df)} datasets ({inserted} inserted, {skipped} skipped)"
        )
...
def load_code_links(conn, batch_size=1000, checkpoint=None):
    ...
    for i in range(start_offset, len(df), batch_size):
        ...
        print(
            f"  Processed {min(i + batch_size, len(df))}/{len(df)} links ({inserted} inserted, {skipped} skipped)"
        )
...
Examples:
  ...
  python load_pwc_data_to_postgres.py --clear-checkpoint
    ...
    parser.add_argument(
        "--fresh", action="store_true", help="Start fresh, ignoring any existing checkpoint"
    )
    ...
    parser.add_argument(
        "--clear-checkpoint", action="store_true", help="Clear checkpoint file and exit"
    )
    ...
    if args.fresh:
        ...
        if any(
            [
                checkpoint.get("papers_complete"),
                checkpoint.get("datasets_complete"),
                checkpoint.get("links_complete"),
                checkpoint.get("eval_complete"),
            ]
        ):
            ...
            papers_status = (
                "✓ Complete"
                if checkpoint.get("papers_complete")
                else f"Resume at {checkpoint.get('papers_offset', 0)}"
            )
            datasets_status = (
                "✓ Complete"
                if checkpoint.get("datasets_complete")
                else f"Resume at {checkpoint.get('datasets_offset', 0)}"
            )
            links_status = (
                "✓ Complete"
                if checkpoint.get("links_complete")
                else f"Resume at {checkpoint.get('links_offset', 0)}"
            )
            eval_status = (
                "✓ Complete"
                if checkpoint.get("eval_complete")
                else f"Resume at {checkpoint.get('eval_offset', 0)}"
            )
